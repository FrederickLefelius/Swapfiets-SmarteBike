...
ha_server = "172.16.2.16"
ha_port = 1883
ha_user = "G4"
ha_password = "gruppe4b"
ha_topic = "homeassistant/sensor/esp32/bike"

try:
    ha_client = MQTTClient(
        client_id="ESP32_Bike_HA",
        server=ha_server,
        port=ha_port,
        user=ha_user,
    ...
    print("Could not connect to Home Assistant:", e)
    ha_connected = False
        ...
        if ticks_diff(now, last_telemetry) > 5000:
            ...
            if ha_connected:
                try:
                    ...
                    ha_client.publish("homeassistant/sensor/esp32/state", payload)
                    print("Sent to HA - Speed:", speed, "Temp:", round(temperature, 1))
                except Exception as e:
